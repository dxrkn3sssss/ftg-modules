...
class RPMod(loader.Module):
    ...
    async def rplistcmd(self, message):
        ...
        await message.edit("<b>• чмок\n• чпок\n• кусь\n• обнять\n• шлеп\n• убить\n• выебать\n• связать\n• ударить\n• уебать\n• отсосать\n• отлизать\n• задушить\n• украсть"
                           "\n• погладить\n• притянуть\n• изнасиловать\n• отпороть\n• наебать\n• поцеловать\n• накурить\n• набухать\n• засосать\n• утопить\n• расстрелять\n• прижать\n• понюхать\n• отдаться\n• покормить\n• кастрировать\n• пнуть\n• пожелать спокойной ночи\n• лизнуть\n• послать нахуй\n• ущипнуть\n• дать чапалаха\n• полюбить\n• признаться в любви\n• трахнуть\n• заебать\n• доебаться\n• дать бан\n• принудить\n• отшлепать\n• разозлила\n• отгрызть кусок плоти и съесть ее на глазах у детей\n• сасать\n• торт</b>")
            ...
            if status is not False:
                if message.sender_id == me.id:
                    ...
                    if message.text.lower() == "отшлепать":
                        await message.edit(f"<a href=tg://user?id={me.id}>{me.first_name}</a> отшлепал(-а) <a href=tg://user?id={user.id}>{user.first_name}</a>")
                    if message.text.lower() == "разозлить":
                        await message.edit(f"<a href=tg://user?id={me.id}>{me.first_name}</a> разозлила(-а) <a href=tg://user?id={user.id}>{user.first_name}</a>")
                    if message.text.lower() == "сасать":
                        await message.edit(f"<a href=tg://user?id={me.id}>{me.first_name}</a> заставил(-а) отсосать <a href=tg://user?id={user.id}>{user.first_name}</a>")
                    if message.text.lower() == "торт":
                        await message.edit(f"<a href=tg://user?id={me.id}>{me.first_name}</a> послал(-а) за тортиком <a href=tg://user?id={user.id}>{user.first_name}</a>")
                    ...
                    if message.text.lower() == "Отгрызть кусок плоти и съесть ее на глазах у детей":
                       await message.edit(f"<a href=tg://user?id={me.id}>{me.first_name}</a> Отгрыз кусок плоти и съел(-а) ее на глазах у детей <a href=tg://user?id={user.id}>{user.first_name}</a>")
